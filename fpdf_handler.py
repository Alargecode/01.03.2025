...
class fpdf_handler(FPDF):
    ...
    def create_pdf(self, max_static, mean_static, max_dynamic, mean_dynamic, sample1, sample2, forces):

        self.set_time()
        self.add_page()
        self.set_font('Times', '', 12)
        if sample2.name == "":
            self.single_table(sample1, max_static, mean_static, max_dynamic, mean_dynamic)
            self.graph_to_pdf(1)
        else:
            self.diff_table(sample1, sample2, max_static, mean_static, max_dynamic, mean_dynamic)
            self.graph_to_pdf(2)
        ...
        #self.close()
        print("pdf created")
        #json_out.dump_time(max_static, mean_static, max_dynamic, mean_dynamic, sample1, sample2, forces, self.date_and_time)
        ...
        try:
            ...
            print("usb kayıt oldu")
        except:
            print("usb kayıt edilmedi")

    def single_table(self, sample, max_dynamic, mean_dynamic, max_static, mean_static):
