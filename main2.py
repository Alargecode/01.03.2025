...
class ScreenTwo(Screen):
    ...
    def start(self):
        ...
        if(durdu == 0):
            ...
            if not gpio.input(stop_switch):
                ...
                speedy_gon = test_speed*39
                speedy_gon = round(speedy_gon)
                md.motor_start(speedy_gon,0)
    ...
    def get_value(self, arg):
        ...
        if(self.ids.dist_current.text == '100.0'):
                self.stop()
        ...
        while getattr(t, "do_run", True):
            ...
            self.ids.dist_current.text = str(round(self.time_*(0.0219*dist_carpan), 1))  
    ...
    def motor_backward(self):
        ...
        if not gpio.input(start_switch):
            self.min_distance_event()
            md.motor_start(100000, 1)
            
    def max_distance_event(self):

        try:
            gpio.setup(stop_switch, gpio.IN, pull_up_down=gpio.PUD_DOWN)
            gpio.add_event_detect(stop_switch, gpio.RISING, callback=self.stop_event, bouncetime=1)
        except:
            pass
    ...
    def min_distance_event(self):
        try:
            gpio.setup(start_switch, gpio.IN, pull_up_down=gpio.PUD_DOWN)
            gpio.add_event_detect(start_switch, gpio.RISING, callback=self.stop_event, bouncetime=1)
...
def language_change_gl(text):
    ...
    if text == 'English':
        ...
        sc_five.ids.reset_factory.text = 'Reset to Factory Settings'
        ...
        sc_five.ids.reset_factory.text = 'Fabrika Ayarlarına Döndür'
        ...
        sc_five.ids.reset_factory.text = 'Сброс к заводским настройкам'
